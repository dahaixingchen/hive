...
-- 因为address是Map的类型，展开来有两个字段，所以需要两个字段来接
...
--创建视图：
	CREATE VIEW [IF NOT EXISTS] [db_name.]view_name 
	  ...
	  AS SELECT ... ;
-- 实例
create view psn22_view as select * from psn2;
-- 进行join操作
 select * from psn2 p left join psn22_view pv on p.name = pv.name;
--查询视图：
...
**hive索引的本质是把需要，索引的字段单独拿出来和对应数据存储的位置也拿出来，放在另外一张表中（hdfs的文件）**，每次新增了数据都要再次重建索引（理论上是提高了查询的速度，但是hdfs文件在磁盘上，本身的查询速度就比较慢）
