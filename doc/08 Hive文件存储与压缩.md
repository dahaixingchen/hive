...
```sql
--1）开启hive中间传输数据压缩功能（默认 false）
	hive (default)>set hive.exec.compress.intermediate=true;
--2）开启mapreduce中map输出压缩功能（默认 false）
	hive (default)>set mapreduce.map.output.compress=true;
--3）设置mapreduce中map输出数据的压缩方式（默认是hadoop中的压缩方式）
...
```sql
--1）开启hive最终输出数据压缩功能（默认 false）
	hive (default)>set hive.exec.compress.output=true;
--2）开启mapreduce最终输出数据压缩（默认 false）
	hive (default)>set mapreduce.output.fileoutputformat.compress=true;
--3）设置mapreduce最终数据输出压缩方式 （默认是hadoop中的压缩方式）
	hive (default)> set mapreduce.output.fileoutputformat.compress.codec = org.apache.hadoop.io.compress.SnappyCodec;
--4）设置mapreduce最终数据输出压缩为块压缩（默认是 RECORD）
...
![行式存储与列式存储](../hive-udf/image/行式存储与列式存储.png)
...
![orc文件格式](../hive-udf/image/orc文件格式.png)
...
![parquet文件格式](../hive-udf/image/parquet文件格式.png)
