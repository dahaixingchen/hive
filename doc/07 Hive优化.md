...
```sql
--查看Hive的数据抓取策略（more这个抓取的优化模式是默认开启的）
...
```sql
--设置本地模式（默认是false，线上的话建议配置到配置文件中）
...
```sql
--设置读取数据量的大小限制（默认就是128M也就是 134217728）
...
```sql
--设置一次SQL计算允许并行执行的job个数的最大值（默认是8个）
...
​		2、Sort By - 对于单个reduce（单个分区）的数据进行排序
​		3、Distribute By - 分区排序，经常和Sort By结合使用（他们都可以用desc，控制升序还是降序）
​		4、Cluster By - 相当于 Sort By + Distribute By
​			（Cluster By不能通过asc、desc的方式指定排序规则；
​				可通过 distribute by column sort by column asc|desc 的方式）

```sql
-- distribute by 和sort by 结合使用(distribute by一定是写在前面的)
select * from psn3 distribute by name sort by name desc;
```
...
```sql
--通过修改以下配置启用自动的mapjoin（默认是true）：
set hive.auto.convert.join = true;
--（该参数为true时，Hive自动对左边的表统计量，如果是小表就加入内存，即对小表使用Map join）

--相关配置参数：
--（大表小表判断的阈值，如果表的大小小于该值则会被加载到内存中运行 默认：25000000大概25M不到的样子）
...
```sql
--通过设置以下参数开启在Map端的聚合（默认是true）：
set hive.map.aggr=true;

--相关配置参数：
--map端group by执行聚合时处理的多少行数据（默认：100000）
hive.groupby.mapaggr.checkinterval;
--进行聚合的最小比例（预先对100000条数据做聚合，若聚合之后的数据量/100000的值大于该配置0.5，则不会聚合，默认是0.5）
hive.map.aggr.hash.min.reduction;
--map端聚合使用的内存的最大值(默认值是0.5 有点看不懂)
...
--是否合并reduce输出文件（默认是false）：
set hive.merge.mapredfiles=true;
--合并文件的大小（默认值256000000）：
...
```sql
--Map数量相关的参数
--一个split的最大值，即每个map处理文件的最大值（默认值 256000000）
set mapred.max.split.size
--一个节点上split的最小值（默认值 1）
set mapred.min.split.size.per.node
--一个机架上split的最小值（默认值 1）
set mapred.min.split.size.per.rack

--Reduce数量相关的参数
--强制指定reduce任务的数量（默认值 1）
set mapred.reduce.tasks
--每个reduce任务处理的数据量（默认值 256000000）
set hive.exec.reducers.bytes.per.reducer
--每个任务最大的reduce数（默认值 1009）
